...
# Script: 04_rq1_decision_tree_reduced_model.R
# Purpose: Fit the reduced RQ1 decision tree model excluding Q28, Q29,
#          Q32, and Q33, evaluate it on a held-out test set, and save outputs.
# ============================================================
...
write.csv(
  grid_results_df,
  file.path(output_rq1_dir, "reduced_model_grid_search_results.csv"),
  row.names = FALSE
)

cat("\nTop 10 reduced-model settings by balanced accuracy:\n")
...
cat("\nTop 10 reduced-model settings by F1 for Satisfied:\n")
...
cat("\nSelected best reduced-model parameters:\n")
print(best_params)

write.csv(
  best_params,
  file.path(output_rq1_dir, "reduced_model_best_params.csv"),
  row.names = FALSE
)
...
cat("\nFinal reduced tree summary:\n")
print(final_tree)

cat("\nFinal reduced tree CP table:\n")
printcp(final_tree)
...
cat("\nInternal best cp from final reduced tree:", best_cp_internal, "\n")

final_tree_pruned <- prune(final_tree, cp = best_cp_internal)

cat("\nPruned final reduced tree summary:\n")
...
cat("\nReduced-model test-set confusion matrix and evaluation:\n")
print(conf_mat)
...
cat("\nSelected reduced-model test metrics:\n")
print(test_metrics)

write.csv(
  test_metrics,
  file.path(output_rq1_dir, "reduced_model_test_metrics.csv"),
  row.names = FALSE
)
...
write.csv(
  confusion_table,
  file.path(output_rq1_dir, "reduced_model_confusion_matrix.csv"),
  row.names = FALSE
)
...
if (!is.null(var_importance)) {
  ...
  cat("\nReduced-model variable importance:\n")
  print(var_importance_df)

  write.csv(
    var_importance_df,
    file.path(output_rq1_dir, "reduced_model_variable_importance.csv"),
    row.names = FALSE
  )
} else {
  cat("\nNo variable importance available. The reduced tree may have no split.\n")
}

png(
  file.path(figure_rq1_dir, "reduced_model_final_tree.png"),
  width = 1500,
  height = 950
)
...
rpart.plot(
  ...
  extra = 104,
  fallen.leaves = TRUE,
  main = "Reduced Decision Tree for Democratic Satisfaction"
)

dev.off()

png(
  file.path(figure_rq1_dir, "reduced_model_unpruned_selected_tree.png"),
  width = 1500,
  height = 950
)
...
rpart.plot(
  ...
  extra = 104,
  fallen.leaves = TRUE,
  main = "Unpruned Reduced Decision Tree"
)
...
write.csv(
  prediction_summary,
  file.path(output_rq1_dir, "reduced_model_prediction_summary.csv"),
  row.names = FALSE
)
...
cat("\nReduced-model threshold analysis:\n")
print(threshold_results)

write.csv(
  threshold_results,
  file.path(output_rq1_dir, "reduced_model_threshold_analysis.csv"),
  row.names = FALSE
)

cat("\nSaved reduced decision tree outputs to:", output_rq1_dir, "\n")
cat("Saved reduced decision tree figures to:", figure_rq1_dir, "\n")